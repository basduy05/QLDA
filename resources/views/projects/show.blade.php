            ...
            <div class="card-strong p-6 bg-gradient-to-br from-white to-slate-50/50">
                ...
                <div class="mt-6 pt-6 border-t border-slate-200/60">
                    ...
                    <div class="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden">
                        <div class="bg-accent h-2.5 rounded-full transition-all duration-500" style="width: {{ $project->tasks_count > 0 ? ($project->tasks()->where('status', 'done')->count() / $project->tasks_count) * 100 : 0 }}%"></div>
                    </div>
                    <p class="text-xs text-slate-500 mt-2 text-center">{{ $project->tasks()->where('status', 'done')->count() }} / {{ $project->tasks_count }} {{ __('tasks completed') }}</p>
                </div>
            </div>
        </div>
